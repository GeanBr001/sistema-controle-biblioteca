...
router.get("/catalog/lookup", async (req, res) => {
  ...
  try {
    const response = await fetch(
      `https://openlibrary.org/isbn/${encodeURIComponent(rawIsbn)}.json`,
      {
        headers: {
          "User-Agent":
            "Biblioteca-TCC/1.0 (contato: admin@biblioteca.local)",
        },
      },
    );
    ...
    console.log("RESPOSTA ISBN:", JSON.stringify(doc, null, 2));
...
if (doc.authors?.length) {
  for (const authorRef of doc.authors) {
...
if (!author) {
  author = "Autor não identificado";
    ...
    const publicationYear = doc.publish_date
      ? String(doc.publish_date).match(/\d{4}/)?.[0] || ""
      : "";
    ...
    const coverImage = doc.covers?.[0]
      ? `https://covers.openlibrary.org/b/id/${doc.covers[0]}-M.jpg?default=false`
